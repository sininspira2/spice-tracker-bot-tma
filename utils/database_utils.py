...
async def timed_database_operation(
...
async def validate_user_exists(
    database, user_id: str, username: str, create_if_missing: bool = True
):
    ...
    if not user and create_if_missing:
        ...
        user = await database.get_user(user_id)
        logger.info(
            f"User created/updated during operation", user_id=user_id, username=username
        )
