...
async def send_response(interaction, content=None, embed=None, view=None, ephemeral=False, use_followup=True):
    ...
    try:
        if use_followup:
            ...
            # Note: ephemeral is not supported for channel.send
            if 'ephemeral' in kwargs:
                del kwargs['ephemeral']
            await interaction.channel.send(**kwargs)

        response_time = time.time() - start_time
        log_kwargs = {k: v is not None for k, v in kwargs.items()}
        log_kwargs['response_time'] = f"{response_time:.3f}s"
        log_kwargs['use_followup'] = use_followup
        logger.info("Response sent successfully", **log_kwargs)
