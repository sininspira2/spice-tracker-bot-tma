    ...
    async def wrapper(interaction, *args, **kwargs):
        ...
        if not hasattr(interaction, "guild") or not interaction.guild:
            try:
                await send_response(
                    interaction,
                    "❌ This command can only be used in a Discord server, not in direct messages.",
                    use_followup=False,
                    ephemeral=True,
                )
            except:
                ...
                logger.warning(
                    f"Command {func.__name__} called in DM context, cannot proceed"
                )
            return
        ...
        try:
            ...
            if not hasattr(interaction, "response") or not hasattr(interaction, "user"):
                logger.warning(
                    f"Invalid interaction object for {func.__name__}, falling back to channel messages"
                )
                use_followup = False
                return
            ...
            await asyncio.wait_for(
                interaction.response.defer(thinking=True), timeout=5.0
            )
            defer_time = time.time() - defer_start
            logger.info(
                f"Interaction deferred successfully",
            ...
            defer_time = time.time() - defer_start
            logger.warning(
                f"Defer timeout for {func.__name__} command",
                user=interaction.user.display_name,
                user_id=interaction.user.id,
                defer_time=f"{defer_time:.3f}s",
            )
        except Exception as defer_error:
            if "Unknown interaction" in str(defer_error) or "NotFound" in str(
                defer_error
            ):
                ...
                defer_time = time.time() - defer_start
                logger.warning(
                    f"Interaction expired for {func.__name__} command",
                    user=interaction.user.display_name,
                    user_id=interaction.user.id,
                    defer_time=f"{defer_time:.3f}s",
                )
        ...
        try:
            ...
            try:
                ...
                if hasattr(interaction, "channel") and interaction.channel:
                    ...
                    logger.warning(
                        f"Interaction invalid for {func.__name__}, skipping error response"
                    )
            except Exception as response_error:
                logger.error(
                    f"Failed to send error response for {func.__name__}: {response_error}"
                )
