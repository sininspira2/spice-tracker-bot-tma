...
class BotLogger:
        ...
        for key, value in kwargs.items():
            if value is not None:
                ...
                if isinstance(value, (int, float)):
                    formatted_value = (
                        f"{value:,}" if isinstance(value, int) else f"{value:.3f}"
                    )
    ...
    def command_success(
        self, command: str, user_id: str, username: str, execution_time: float, **kwargs
    ):
    ...
    def command_error(
        self, command: str, user_id: str, username: str, error: str, **kwargs
    ):
    ...
    def rate_limit_hit(self, command: str, user_id: str, username: str):
        ...
        self.warning(f"Rate limit hit: {command}", user=username, user_id=user_id)

    def permission_denied(
        self, command: str, user_id: str, username: str, required_permission: str
    ):
    ...
    def bot_event(self, event: str, **kwargs):
        ...
        self.info(f"Bot event: {event}", **kwargs)

    def database_operation(self, operation: str, table: str, success: bool, **kwargs):
        """Log database operations"""
        if success:
            self.info(f"Database {operation} on {table}", **kwargs)
        else:
            self.error(f"Database {operation} failed on {table}", **kwargs)
