...
def register_commands():
    ...
    async def spicesplit_cmd(interaction: discord.Interaction, total_sand: int, participants: int, harvester_percentage: float = 25.0):
...
async def leaderboard(interaction: discord.Interaction, limit: int = 10):
    ...
    if not leaderboard_data:
        embed = (EmbedBuilder("🏆 Melange Refining Leaderboard", color=0x95A5A6, timestamp=interaction.created_at)
                 .set_description("🏜️ No refiners found yet! Be the first to start depositing sand with `/spicesolo`."))
        await interaction.response.send_message(embed.build())
        return
...
async def spicesplit(interaction: discord.Interaction, total_sand: int, participants: int, harvester_percentage: float = 25.0):
    ...
    # Use environment variable if default value is used
    if harvester_percentage == 25.0:
