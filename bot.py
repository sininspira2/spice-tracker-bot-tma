...
async def on_ready():
    ...
    try:
        if bot.user:
            logger.bot_event(
                f"Bot started - {bot.user.name} ({bot.user.id}) in {len(bot.guilds)} guilds"
            )
        ...
        try:
            ...
            logger.bot_event(
                "Database connection verified", db_init_time=f"{db_init_time:.3f}s"
            )
            ...
            logger.error(
                f"Database connection failed in {db_init_time:.3f}s",
                error=str(error),
                error_type=type(error).__name__,
            )
            logger.debug(
                "Database connection failure traceback",
        ...
        logger.info(
            "Command registration completed", register_time=f"{register_time:.3f}s"
        )
        ...
        if auto_sync:
            try:
                ...
                for guild in bot.guilds:
                    try:
                        ...
                        logger.info(
                            ...
                            commands_synced=len(guild_synced),
                        )
                ...
                logger.info(
                    "Auto-sync completed",
                    global_commands=len(global_synced),
                    guild_commands=guild_sync_count,
                )

            except Exception as sync_error:
                logger.error("Auto-sync failed", error=str(sync_error))
                logger.debug(
                    "Auto-sync failure traceback", traceback=traceback.format_exc()
                )
        ...
        logger.bot_event(
            f"Bot startup completed",
            total_startup_time=f"{total_startup_time:.3f}s",
            db_init_time=f"{db_init_time:.3f}s",
            guild_count=len(bot.guilds),
        )
        logger.info(
            "Bot startup completed", total_startup_time=f"{total_startup_time:.3f}s"
        )

    except Exception as error:
        total_startup_time = time.time() - bot_start_time
        logger.error(
            "CRITICAL ERROR in on_ready",
            error=str(error),
            error_type=type(error).__name__,
            startup_time=f"{total_startup_time:.3f}s",
        )
        logger.debug("on_ready failure traceback", traceback=traceback.format_exc())
        logger.error(
            f"Critical error in on_ready: {error}",
            startup_time=f"{total_startup_time:.3f}s",
        )
...
def register_commands():
    ...
    @bot.tree.command(
        name=cmd_name("sand"),
        description="Convert spice sand into melange (50:1 ratio)",
    )
    ...
    @bot.tree.command(
        name=cmd_name("calc"),
        description="Estimate melange from a sand amount (no database update)",
    )
    ...
    # Leaderboard command
    @bot.tree.command(
        name=cmd_name("leaderboard"),
        description="Display top spice refiners by melange production",
    )
    @app_commands.describe(
        limit="Number of top refiners to display (5-25, default: 10)"
    )
    ...
    @bot.tree.command(
        name=cmd_name("split"),
        description="Split spice sand among expedition members and convert to melange",
    )
    @app_commands.describe(
    ...
    # Help command
    @bot.tree.command(
        name=cmd_name("help"), description="Show all available spice tracking commands"
    )
    ...
    @bot.tree.command(
        name=cmd_name("perms"),
        description="Show your permission status and matched roles",
    )
    ...
    @app_commands.describe(
        confirm="Confirm that you want to delete all refinery data (True/False)"
    )
    ...
    # Ledger command
    @bot.tree.command(
        name=cmd_name("ledger"), description="View your complete spice harvest ledger"
    )
    ...
    # Expedition command
    @bot.tree.command(
        name=cmd_name("expedition"), description="View details of a specific expedition"
    )
    @app_commands.describe(expedition_id="ID of the expedition to view")
    async def expedition_cmd(
        interaction: discord.Interaction, expedition_id: int
    ):  # noqa: F841
    ...
    @app_commands.describe(
        destination="Destination for water delivery (default: DD base)"
    )
    async def water_cmd(
        interaction: discord.Interaction, destination: str = "DD base"
    ):  # noqa: F841
        await water(interaction, destination)

    # DB Sync command
    @bot.tree.command(
        name=cmd_name("dbsync"),
        description="Manually resynchronizes all database primary key sequences (Bot Owner only)",
    )
    ...
    @bot.tree.command(
        name=cmd_name("sync"), description="Sync slash commands (Bot Owner Only)"
    )
                    ...
                    if requester_mention:
                        ...
                        try:
                            ...
                            if requester_mention.startswith(
                                "<@"
                            ) and requester_mention.endswith(">"):
                                ...
                                if requester:
                                    ...
                                    notification_embed.add_field(
                                        name="📍 Destination",
                                        value=embed.description.replace(
                                            "**Location:** ", ""
                                        ),
                                        inline=False,
                                    )
                            ...
                            logger.warning(
                                f"Could not send notification to requester: {e}"
                            )

                        # Log the completion
                        logger.info(
                            f"Water delivery completed by admin",
    ...
    class HealthHandler(http.server.BaseHTTPRequestHandler):
        def do_GET(self):
            ...
            if self.path == "/health":
                ...
                logger.info(
                    f"Health check request completed",
                ...
                request_time = time.time() - request_start
                logger.info(
                    f"Ping request completed", request_time=f"{request_time:.3f}s"
                )
                ...
                request_time = time.time() - request_start
                logger.warning(
                    f"Invalid health check request",
                    path=self.path,
                    request_time=f"{request_time:.3f}s",
                )
...
if __name__ == "__main__":
    ...
    def keep_alive():
        ...
        while True:
            try:
                ...
                ping_count += 1
                logger.warning(
                    f"Keep-alive ping failed", ping_count=ping_count, error=str(e)
                )
        ...
        if database_url:
            ...
            if parsed.scheme.startswith("postgres"):
                ...
                if is_supabase:
                    ...
                    valid_host = hostname.startswith("db.") and (
                        hostname.endswith(".supabase.co")
                        or hostname.endswith(".supabase.net")
                    )
                    if not valid_host:
                        errors.append(
                            "host must be 'db.[PROJECT].supabase.co' (or .supabase.net)"
                        )
                    ...
                    if not parsed.path or parsed.path == "/":
                        errors.append(
                            "database name missing in path (e.g., '/postgres')"
                        )

                    if errors:
                        for issue in errors:
                            logger.error(
                                "Invalid Supabase DATABASE_URL: " + issue,
                                hostname=hostname,
                            )
                        logger.error(
                            "Example format: postgresql+asyncpg://postgres:[PASSWORD]@db.[PROJECT].supabase.co:5432/postgres"
                        )
                        exit(1)
    except Exception as e:
        logger.warning(
            "DATABASE_URL validation failed; proceeding anyway", error=str(e)
        )
    ...
    if not token:
        ...
        logger.error(
            "Please set the DISCORD_TOKEN environment variable in Fly.io or your .env file"
        )
        exit(1)
    ...
    try:
        ...
        logger.error(
            f"Bot startup failed", startup_time=f"{startup_time:.3f}s", error=str(e)
        )
        raise e
