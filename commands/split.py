...
async def split(
    ...
    try:
        ...
        if total_sand < 1:
            await send_response(
                interaction,
                "❌ Total spice sand must be at least 1.",
        ...
        if not 0 <= guild <= 100:
            await send_response(
                interaction,
                "❌ Guild cut percentage must be between 0 and 100.",
                use_followup=use_followup,
                ephemeral=True,
            )
            return
        ...
        if user_cut is not None and not 0 <= user_cut <= 100:
            await send_response(
                interaction,
                "❌ User cut percentage must be between 0 and 100.",
                use_followup=use_followup,
                ephemeral=True,
            )
            return
        ...
        if user_cut is not None:
            if any(percentage_str for _, percentage_str in matches):
                await send_response(
                    interaction,
                    "❌ You cannot provide individual percentages when using `user_cut`.",
                    use_followup=use_followup,
                    ephemeral=True,
                )
                return
            ...
            for user_id, percentage_str in matches:
                if percentage_str:
                    ...
                    if not 0 <= percentage <= 100:
                        await send_response(
                            interaction,
                            f"❌ Percentage {percentage} is invalid. Must be between 0-100.",
                            use_followup=use_followup,
                            ephemeral=True,
                        )
                        return
        ...
        if total_percentage > 100:
            await send_response(
                interaction,
                f"❌ Total user percentages ({total_percentage}%) cannot exceed 100%.",
                use_followup=use_followup,
                ephemeral=True,
            )
            return
        ...
        if is_all_percentage:
            ...
            await send_response(
                interaction, warning_message, use_followup=use_followup, ephemeral=True
            )
        ...
        if is_all_percentage:
            ...
            melange_for_equal_split = (
                melange_for_players - melange_given_to_percentage_users
            )
            if equal_split_users:
                ...
                for user_id in equal_split_users:
                    ...
                    equal_percentage = (
                        (user_melange / total_melange) * 100 if total_melange > 0 else 0
                    )
        ...
        for user_id, melange, percentage in user_distributions:
            if user_id in unique_distributions:
                ...
                unique_distributions[user_id] = (
                    existing_melange + melange,
                    existing_pct + percentage,
                )
            else:
        ...
        if not unique_distributions:
            await send_response(
                interaction,
                "❌ No valid users found to split with.",
                use_followup=use_followup,
                ephemeral=True,
            )
            return
        ...
        total_user_melange = sum(
            melange for melange, _ in unique_distributions.values()
        )
        ...
        actual_guild_percentage = (
            (total_guild_sand_value / total_sand) * 100 if total_sand > 0 else 0
        )
        ...
        if not expedition_id:
            await send_response(
                interaction,
                "❌ Failed to create expedition record.",
                use_followup=use_followup,
                ephemeral=True,
            )
            return
        ...
        for user_id, (user_melange, user_percentage) in unique_distributions.items():
            try:
                ...
                try:
                    ...
                    logger.debug(
                        f"User {user_id} not found in guild, trying client fetch: {e}"
                    )
                    try:
                        ...
                        logger.warning(
                            f"User {user_id} not found via client fetch: {e}"
                        )
            ...
            if user_melange > 0:
                ...
                # Format for display
                percentage_text = (
                    f" ({user_percentage:.1f}%)" if user_percentage > 0 else ""
                )
                participant_details.append(
                    f"**{display_name}**: {user_melange:,} melange{percentage_text}"
                )
        ...
        logger.info(
            f"Expedition {expedition_id} created by {interaction.user.display_name} ({interaction.user.id})",
            total_sand=total_sand,
            total_melange=total_melange,
            guild_melange=guild_melange,
            guild_sand=guild_sand,
            participants=len(unique_distributions),
        )
        ...
        try:
            await send_response(
                interaction,
                "❌ An error occurred while processing the split. Please try again.",
                use_followup=use_followup,
                ephemeral=True,
            )
