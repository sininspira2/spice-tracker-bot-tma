...
async def expedition(
    interaction, command_start, expedition_id: int, use_followup: bool = True
):
    ...
    try:
        ...
        if not expedition_data:
            await send_response(
                interaction,
                "❌ Expedition not found or you don't have access to it.",
        ...
        total_time = time.time() - command_start
        logger.error(
            f"Error in expedition command: {error}",
        ...
        await send_response(
            interaction,
            "❌ An error occurred while fetching expedition details.",
            use_followup=use_followup,
            ephemeral=True,
        )
