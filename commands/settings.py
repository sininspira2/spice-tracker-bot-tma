...
class Settings(app_commands.Group):
        ...
        *,
        ...
        if roles_str is None:
            ...
            try:
                ...
                embed = build_status_embed(
                    title=f"✅ {title} Updated", description=description, color=0x00FF00
                )
                await self.send_response(interaction, embed=embed.build())
                ...
                log_command_metrics(
                    ...
                    new_value=parsed_roles,
                )

            except Exception as e:
                logger.error(
                    f"Error setting {setting_key}: {e}",
                    user_id=str(interaction.user.id),
                )
                await self.send_response(
                    interaction, f"❌ An error occurred: {e}", ephemeral=True
                )

    @app_commands.command(
        name="admin_roles", description="Set or view the roles with admin permissions."
    )
    @app_commands.describe(
        roles="Comma-separated role IDs or @role mentions. Leave blank to view, or send empty to clear."
    )
    async def admin_roles(
        self, interaction: discord.Interaction, roles: Optional[str] = None
    ):
    ...
    @app_commands.command(
        name="officer_roles",
        description="Set or view the roles with officer permissions.",
    )
    @app_commands.describe(
        roles="Comma-separated role IDs or @role mentions. Leave blank to view, or send empty to clear."
    )
    async def officer_roles(
        self, interaction: discord.Interaction, roles: Optional[str] = None
    ):
    ...
    @app_commands.command(
        name="user_roles", description="Set or view the roles allowed to use the bot."
    )
    @app_commands.describe(
        roles="Comma-separated role IDs or @role mentions. Leave blank to view, or send empty to clear."
    )
    ...
    @app_commands.command(
        name="landsraad",
        description="Manage the landsraad bonus for melange conversion",
    )
    @app_commands.describe(
    ...
    async def landsraad(
        ...
        try:
            ...
            if action == "status":
                ...
                is_active = (
                    landsraad_status_str and landsraad_status_str.lower() == "true"
                )
                ...
                fields = {
                    ...
                    "💡 Effect": (
                        "37.5 sand = 1 melange" if is_active else "50 sand = 1 melange"
                    ),
                }
                ...
                fields = {
                    ...
                    "⚙️ Conversion Rate": rate_text,
                    "💡 Effect": (
                        "37.5 sand = 1 melange" if new_status else "50 sand = 1 melange"
                    ),
                }
            ...
            logger.error(
                f"Error in settings landsraad command: {error}",
                user_id=str(interaction.user.id),
            ...
            await self.send_response(
                interaction,
                f"❌ An error occurred while managing landsraad bonus: {error}",
                ephemeral=True,
            )
    ...
    async def _handle_setting(
        ...
        *,
        ...
        if value is None:
            ...
            embed = build_status_embed(
                title=view_title, description=description, color=0x3498DB
            )
            await self.send_response(interaction, embed=embed.build())
        else:
            # Set logic
    ...
    @app_commands.command(
        name="user_cut",
        description="Set or view the default user cut percentage for /split.",
    )
    @app_commands.describe(
        value="The default percentage for each user in a split (0 to unset)."
    )
    async def user_cut(
        self,
        interaction: discord.Interaction,
        ...
        await self._handle_setting(
            ...
            view_description_formatter=lambda v: (
                f"Current default user cut is **{v}%**."
                if v is not None
                else "Current default user cut is **Not set (optional in /split)**."
            ),
    ...
    @app_commands.command(
        name="guild_cut",
        description="Set or view the default guild cut percentage for /split.",
    )
    @app_commands.describe(
        value="The default percentage for the guild in a split (0 for default 10%)."
    )
    async def guild_cut(
        self,
        interaction: discord.Interaction,
        ...
        await self._handle_setting(
            ...
            value_for_update=lambda v: v,
            log_name="GuildCut",
            set_embed_fields=lambda v: (
                {
                    "ℹ️ Note": "A value of 0 unsets the global default, reverting to the bot's default of 10%."
                }
                if v == 0
                else None
            ),
    ...
    @app_commands.command(
        name="region", description="Set or view the guild's primary region."
    )
    ...
    async def region(
        self, interaction: discord.Interaction, region: app_commands.Choice[str] = None
    ):
