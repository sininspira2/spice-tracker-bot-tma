...
async def calculate_sand(interaction, amount: int, landsraad_bonus: bool = False, use_followup: bool = True):
    ...
    # Validate amount
    if amount < 1:
        await send_response(interaction, "❌ Amount must be 1 or larger.", use_followup=use_followup, ephemeral=True)
        return
    ...
    # Calculate melange
    calculated_melange = amount / sand_per_melange if sand_per_melange > 0 else 0

    # Build response
    description = f"🎉 **{calculated_melange:,.2f} melange** would be generated from **{amount:,} sand**."
