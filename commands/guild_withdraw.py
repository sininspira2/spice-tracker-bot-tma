...
COMMAND_METADATA = {
    'aliases': [],
    'description': "Withdraw melange from guild treasury to give to a user (Admin only)",
    'params': {
    ...
    try:
        ...
        if current_melange < amount:
            await send_response(interaction,
                ...
                f"**Available:** {current_melange:,} melange\n"
                f"**Requested:** {amount:,} melange\n"
                f"**Shortfall:** {amount - current_melange:,} melange",
        ...
        fields = {
            "💸 Transaction": f"**Recipient:** {user.display_name} | **Amount:** {amount:,} melange | **Admin:** {interaction.user.display_name}",
            "🏛️ Treasury": f"**Previous:** {current_melange:,} | **New:** {updated_treasury.get('total_melange', 0):,} | **Available:** {updated_treasury.get('total_melange', 0):,}"
        }

        embed = build_status_embed(
            title="✅ Guild Withdrawal Completed",
            description=f"💰 **{amount:,} melange** transferred from guild treasury to **{user.display_name}**",
            color=0x00FF00,
            fields=fields,
        ...
        logger.info(f"Guild withdrawal: {amount:,} melange from treasury to {user.display_name} ({user.id}) by {interaction.user.display_name} ({interaction.user.id})")
