...
def pending_mocks(mocker):
    ...
    mock_db = mocker.patch('commands.pending.get_database', return_value=AsyncMock()).return_value
    mock_send_response = mocker.patch('commands.pending.send_response', new_callable=AsyncMock)
    mock_build_embed = mocker.patch('commands.pending.build_status_embed', return_value=MagicMock(build=lambda: "embed_obj"))

    async def mock_timed_db_op(name, coro_func, *args, **kwargs):
        result = await coro_func(*args, **kwargs)
        return result, 0.1
    mocker.patch('commands.pending.timed_database_operation', side_effect=mock_timed_db_op)
