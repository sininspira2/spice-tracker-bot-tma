...
def pay_mocks(mocker):
    ...
    mock_db = mocker.patch('commands.pay.get_database', return_value=AsyncMock()).return_value
    mock_send_response = mocker.patch('commands.pay.send_response', new_callable=AsyncMock)
    mock_build_embed = mocker.patch('commands.pay.build_status_embed', return_value=MagicMock(build=lambda: "embed_obj"))
...
async def test_pay_command_success_full_amount(mock_interaction, pay_mocks):
    ...
    db_mock.get_user_pending_melange.return_value = {'pending_melange': 500, 'total_melange': 1000, 'paid_melange': 500}
    db_mock.pay_user_melange.return_value = 500 # Amount paid

    # When
    await pay.__wrapped__(mock_interaction, command_start=0, user=target_user, amount=None, use_followup=True)

    # Then
    db_mock.get_user_pending_melange.assert_called_once_with(str(target_user.id))
    db_mock.pay_user_melange.assert_called_once_with(str(target_user.id), target_user.display_name, 500, str(mock_interaction.user.id), mock_interaction.user.display_name)
...
async def test_pay_command_no_pending_melange(mock_interaction, pay_mocks):
    ...
    target_user = MagicMock(id=456, display_name="Harvester")
    db_mock.get_user_pending_melange.return_value = {'pending_melange': 0, 'total_melange': 500, 'paid_melange': 500}
...
async def test_pay_command_amount_exceeds_pending(mock_interaction, pay_mocks):
    ...
    # When
    await pay.__wrapped__(mock_interaction, command_start=0, user=target_user, amount=600, use_followup=True)
