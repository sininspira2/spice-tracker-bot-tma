...
def refinery_mocks(mocker):
    ...
    mock_send_response = mocker.patch(
        "commands.refinery.send_response", new_callable=AsyncMock
    )
    mocker.patch(
        "commands.refinery.build_status_embed",
        return_value=MagicMock(build=lambda: "embed_obj"),
    )
    mocker.patch(
        "commands.refinery.build_info_embed",
        return_value=MagicMock(build=lambda: "embed_obj"),
    )

    # Mock validate_user_exists as it's the source of user data
    mock_validate_user = mocker.patch(
        "commands.refinery.validate_user_exists", new_callable=AsyncMock
    )
...
async def test_refinery_command_existing_user(mock_interaction, refinery_mocks):
    ...
    send_response_mock.assert_called_once_with(
        mock_interaction, embed="embed_obj", use_followup=True, ephemeral=True
    )
