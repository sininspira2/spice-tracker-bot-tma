...
class TestResetCommand:
    ...
    async def test_reset_confirm_false(self, mock_interaction, test_database):
        with (
            patch("commands.reset.send_response", new_callable=AsyncMock) as mock_send,
            patch.object(
                test_database, "reset_all_stats", new_callable=AsyncMock
            ) as mock_reset_stats,
        ):
            await reset.__wrapped__(
    ...
    @pytest.mark.asyncio
    async def test_reset_confirm_true_then_confirm(
        ...
        with (
            ...
            with patch(
                "commands.reset.timed_database_operation",
    ...
    @pytest.mark.asyncio
    async def test_reset_confirm_true_then_cancel(
        self, mock_interaction, test_database
    ):
        mock_interaction.edit_original_response = AsyncMock()
        with (
