...
def leaderboard_mocks(mocker):
    ...
    mock_send_response = mocker.patch('commands.leaderboard.send_response', new_callable=AsyncMock)
    mock_build_leaderboard = mocker.patch('commands.leaderboard.build_leaderboard_embed', return_value=MagicMock(build=lambda: "leaderboard_embed"))
    mock_build_info = mocker.patch('commands.leaderboard.build_info_embed', return_value=MagicMock(build=lambda: "info_embed"))

    # Mock the timed db operation
    async def mock_timed_db_op(name, coro_func, *args, **kwargs):
        result = await coro_func(*args, **kwargs)
        return result, 0.1
    mocker.patch('commands.leaderboard.timed_database_operation', side_effect=mock_timed_db_op)
...
async def test_leaderboard_command_success(mock_interaction, leaderboard_mocks):
    # Given
    db_mock, send_response_mock, build_leaderboard_mock, build_info_mock = leaderboard_mocks
    leaderboard_data = [
    ...
    # When
    await leaderboard.__wrapped__(mock_interaction, command_start=0, limit=5, use_followup=True)
    ...
    send_response_mock.assert_called_once_with(mock_interaction, embed="leaderboard_embed", use_followup=True)
...
async def test_leaderboard_command_no_data(mock_interaction, leaderboard_mocks):
    ...
    send_response_mock.assert_called_once_with(mock_interaction, embed="info_embed", use_followup=True)
...
async def test_leaderboard_invalid_limit(mock_interaction, leaderboard_mocks):
    ...
    # When
    await leaderboard.__wrapped__(mock_interaction, command_start=0, limit=200, use_followup=True)
